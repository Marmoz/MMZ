...
<?php if ( $lenseignant['user_id'] == 0) {
  ?>  
    <a href="<?= base_url('Examples/dakhlaTeacher') ?>/<?=$lenseignant['idTeacher'] ?>" class="btn btn-info">Creer un Compte</a>
    ...
    if ($auth_role=='admin') { ?>
            <a class="btn btn-success" role="button" href="<?= base_url('Enseignant/add') ?>">Ajouter un Enseignant</a>
                        ...
                        <a class="btn btn-warning" role="button" href="<?= base_url('teach/add') ?>">Affecter une Matière à un Enseignant</a>
                          <a class="btn btn-info" role="button" href="<?= base_url('teach/index') ?>">Détail matières</a>
